...
def sample_boundary_points(polygons, points_per_meter=0.03, min_points_per_poly=4):
    """Sample points along polygon boundaries.

    - `points_per_meter` controls density; approximate number of points per unit length.
    - returns Nx2 numpy array of (x,y) coordinates.    python Resultados_artigo/polygon_representation.py
    """
    samples = []
...
def build_gvd_by_clipping(polygons, boundary_samples, bbox_padding=5.0):
    ...
    def add_linestring_clipped(ls):
        ...
        if inter.geom_type == 'LineString':
            ...
            # Skip other geometry types like Point, MultiPoint, etc.
            return
        for g in geoms:
            if g.length == 0:
                continue
...
def plot_gvd_graph(G, free_space, obstacles_union, out_png, show=True, overlay_graph=None, obs_points=None):
    ...
    if obstacles_union is not None and not obstacles_union.is_empty:
        if obstacles_union.geom_type == 'Polygon':
            ...
            ax.fill(xs, ys, facecolor='#e69292', edgecolor='#b83a3a', alpha=0.8)
        else:
    ...
    ax.scatter(vx, vy, c='#b83a3a', s=100, zorder=10)

    for u, v in G.edges():
        x1, y1 = G.nodes[u]['pos']
        x2, y2 = G.nodes[v]['pos']
        ax.plot([x1, x2], [y1, y2], c='#68a663', linewidth=3.0, zorder=5)
...
def main():
    ...
    parser.add_argument('--points-density', type=float, default=0.1, help='Boundary sampling density (points per meter)')
    ...
    obstacles = [
                {"pos": (15, 82), "size": (20, 15)},
                {"pos": (72, 72), "size": (35, 30)},
                {"pos": (32, 40), "size": (25, 25)},
                {"pos": (48, 24), "size": (20, 14)},
                {"pos": (72, 17), "size": (35, 18)}
            ]
    ...
    boundary_samples = sample_boundary_points(polygons, points_per_meter=args.points_density, min_points_per_poly=4)
    print(f'Sampled {len(boundary_samples)} boundary points')

    # Compute obstacles union for plotting
    obstacles_union = unary_union(polygons) if polygons else Polygon()

    # Plot unclipped Voronoi
    vor = Voronoi(boundary_samples)
    xmin, xmax, ymin, ymax = workspace_bbox_from_polygons(polygons, padding=10.0)
    fig, ax = plt.subplots(figsize=(10, 8))
    ...
    if obstacles_union is not None and not obstacles_union.is_empty:
        if obstacles_union.geom_type == 'Polygon':
            xs, ys = obstacles_union.exterior.xy
            ax.fill(xs, ys, facecolor='#e69292', edgecolor='#b83a3a', alpha=0.8, linewidth=2.0)
        else:
            for geom in obstacles_union.geoms:
                xs, ys = geom.exterior.xy
                ax.fill(xs, ys, facecolor='#e69292', edgecolor='#b83a3a', alpha=0.8, linewidth=2.0)

    # Plot all Voronoi ridges
    center_points = vor.points
    for (pidx0, pidx1), ridge_vertices in zip(vor.ridge_points, vor.ridge_vertices):
        if -1 in ridge_vertices:
            finite_idx = next((rv for rv in ridge_vertices if rv != -1), None)
            ...
            for i in range(len(coords) - 1):
                a = coords[i]
                b = coords[i + 1]
                ax.plot([a[0], b[0]], [a[1], b[1]], c='#68a663', linewidth=3.0, zorder=5)
        else:
            v0 = vor.vertices[ridge_vertices[0]]
            v1 = vor.vertices[ridge_vertices[1]]
            ax.plot([v0[0], v1[0]], [v0[1], v1[1]], c='#68a663', linewidth=3.0, zorder=5)

    # Plot boundary samples as dots
    ax.scatter(boundary_samples[:,0], boundary_samples[:,1], c='#b83a3a', s=100, zorder=10)

    ax.set_aspect('equal', adjustable='box')
    ax.set_title('Unclipped Voronoi Diagram of Obstacle Boundaries')
    ax.set_xlabel('X (meters)', fontsize=16)
    ax.set_ylabel('Y (meters)', fontsize=16)
    ax.tick_params(axis='both', which='major', labelsize=14)
    plt.tight_layout()
    out_png_unclipped = os.path.join(RESULTS_DIR, 'gvd_equipment_unclipped.png')
    plt.savefig(out_png_unclipped, dpi=300)
    plt.show()
    plt.close(fig)
    print(f'Saved unclipped plot to {out_png_unclipped}')
    ...
    if args.overlay:
        try:
            ...
            G_orig, aabbs, obs_pts_list, obstacles2 = build_graph(file_path=None, sheet_name=None, margin=args.margin, threshold=30, plotting=False)
    ...
    plot_gvd_graph(G, free_space, obstacles_union, out_png, show=True, overlay_graph=overlay_graph, obs_points=obs_points)
