...
local file_name = "simulated.csv"
local file
file = io.open(file_name, "a")
if not file then
  gcs:send_text(MAV_SEVERITY.INFO, string.format("Could not create file"))
end
-- write the csv header
file:write('Time Stamp(ms), taux, tauy, taun, u, v, r\n')
file:flush()
local u = 2
local v = 3
local r = 4
...
local function write_to_file()
  ...
  if not file then
    gcs:send_text(MAV_SEVERITY.INFO, string.format("Could not open file"))
  end
...
-- Função para obter a velocidade no referencial da embarcação
local function get_velocity_from_groundspeed()
  local groundspeed_vector = ahrs:groundspeed_vector()
  local groundspeed_north = groundspeed_vector:x()
  local groundspeed_east = groundspeed_vector:y()
  local yaw = ahrs:get_yaw()  -- Usar a função get_yaw

  -- Transformar velocidade de norte/leste para x/y no referencial da embarcação
  local vel_x = groundspeed_north * math.cos(yaw) + groundspeed_east * math.sin(yaw)
  local vel_y = -groundspeed_north * math.sin(yaw) + groundspeed_east * math.cos(yaw)

  return vel_x, vel_y, 0 -- Assumindo que a velocidade vertical é zero
end
...
-- Função para mapear valores relativos (-100 a 100) para PWM absoluto (1100 a 1900)
local function mapRelativeToAbsolute(rel)
  return 1500 + rel * 4
end

-- Definição das sequências de PWM usando valores relativos
local pwm_sequences_1 = {
  { pwm = {40, -70, 90, -50}, duration = 1.9 },    -- Diferença no controle de velocidade
  { pwm = {50, -80, -100, 60}, duration = 1.7 },      -- Ré com diferença de torque
  { pwm = {-80, 60, 40, -100}, duration = 1.8 },      -- Controle dinâmico de curva
  { pwm = {90, -50, -60, 40}, duration = 1.6 },      -- Ré desbalanceada
  { pwm = {-70, 100, 60, -80}, duration = 2.0 },      -- Ajuste dinâmico de potência
  { pwm = {-100, 40, 90, -50}, duration = 1.5 },  -- Teste de resposta de giro
  { pwm = {30, -70, -100, 90}, duration = 1.9 },      -- Movimento reverso complexo
  { pwm = {-50, 90, 60, -70}, duration = 1.6 },      -- Ajuste fino de velocidade
  { pwm = {100, -80, -50, 40}, duration = 1.7 },      -- Teste de estabilidade
  { pwm = {-70, 50, 100, -90}, duration = 1.8 }       -- Diferenciação nas acelerações
}

local sequence_index = 1
local sequence_timer = pwm_sequences_1[sequence_index].duration

local function guidedMode()
  local seq = pwm_sequences_1[sequence_index]
  if seq then
    local pwm_values = seq.pwm
    -- Converte os valores relativos para valores absolutos e aplica para cada canal
    SRV_Channels:set_output_pwm_chan_timeout(0, mapRelativeToAbsolute(pwm_values[1]), 300)
    SRV_Channels:set_output_pwm_chan_timeout(1, mapRelativeToAbsolute(pwm_values[2]), 300)
    SRV_Channels:set_output_pwm_chan_timeout(2, mapRelativeToAbsolute(pwm_values[3]), 300)
    SRV_Channels:set_output_pwm_chan_timeout(3, mapRelativeToAbsolute(pwm_values[4]), 300)

    -- Atualiza o timer da sequência (0.2 segundos por ciclo)
    sequence_timer = sequence_timer - 0.2
    if sequence_timer <= 0 then
      sequence_index = sequence_index + 1
      if pwm_sequences_1[sequence_index] then
         sequence_timer = pwm_sequences_1[sequence_index].duration
      else
         gcs:send_text(MAV_SEVERITY.INFO, "Sequência PWM concluída.")
         -- Opcional: reiniciar a sequência se necessário
         -- sequence_index = 1
         -- sequence_timer = pwm_sequences_1[1].duration
      end
    end
  else
    gcs:send_text(MAV_SEVERITY.WARNING, "Nenhuma sequência disponível.")
...
local function update()
  ...
  elseif vehicle:get_mode() == DRIVING_MODES.GUIDED then
    accumulated_time = accumulated_time + 0.2  -- 0.2 segundos (200ms)
    guidedMode()
    -- Obter as velocidades
    local vel_x, vel_y, vel_z = get_velocity_from_groundspeed()
    local yaw_save = ahrs:get_yaw()  -- Usar a função get_yaw
    local gyro_x, gyro_y, gyro_z = ahrs:get_gyro()

    -- Verificar se algum valor é nil e substituir por zero
    vel_x = vel_x or 0
    vel_y = vel_y or 0
    vel_z = vel_z or 0
    gyro_z = gyro_z or 0
    interesting_data[te] = accumulated_time
    interesting_data[u] = vel_x
    interesting_data[v] = vel_y
    interesting_data[r] = yaw_save
    interesting_data[z] = gyro_z
